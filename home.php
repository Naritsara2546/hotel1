...
$rooms = [];
$sqlRooms = "SELECT * FROM meeting_rooms ORDER BY name ASC";
        ...
        <?php foreach ($rooms as $room): ?>
          <div class="room-card" data-bs-toggle="modal" data-bs-target="#roomModal-<?= htmlspecialchars($room['room_code']) ?>">
            <?php if (!empty($room['image']) && file_exists('uploads/' . $room['image'])): ?>
              <img class="room-image" src="uploads/<?= htmlspecialchars($room['image']) ?>" alt="<?= htmlspecialchars($room['name']) ?>" />
            <?php else: ?>
            ...
            <div class="modal-dialog modal-lg">
              <div class="modal-content">
                <div class="modal-header">
                  <h5 class="modal-title" id="roomModalLabel-<?= htmlspecialchars($room['room_code']) ?>"><?= htmlspecialchars($room['name']) ?></h5>
                  <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body">
                  <?php if (!empty($room['image']) && file_exists('uploads/' . $room['image'])): ?>
                    <img class="img-fluid mb-3" src="uploads/<?= htmlspecialchars($room['image']) ?>" alt="<?= htmlspecialchars($room['name']) ?>" />
                  <?php endif; ?>
